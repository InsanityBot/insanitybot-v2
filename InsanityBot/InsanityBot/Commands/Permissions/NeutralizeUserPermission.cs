...
namespace InsanityBot.Commands.Permissions
{
    [Group("permission")]
    [Aliases("permissions")]
    public class NeutralizeUserPermission : BaseCommandModule
    {
        [Command("neutralize")]
        ...
        private async Task ParseNeutralizePermission(CommandContext ctx, DiscordMember member, String args)
        {
            ...
            catch (Exception e)
            {
                ...
                {
                    Description = GetFormattedString(InsanityBot.LanguageConfig["insanitybot.permission.error.could_not_parse_neutral"],
                        ctx, member),
                    Color = DiscordColor.Red,
                    Footer = new()
                    {
        ...
        private async Task ExecuteNeutralizePermission(CommandContext ctx, DiscordMember member, Boolean silent, String permission)
        {
            ...
            try
            {
                ...
                embedBuilder = new()
                {
                    ...
                    Description = GetFormattedString(InsanityBot.LanguageConfig["insanitybot.permission.permission_neutralized"], ctx, member, permission)
                };
            ...
            catch(Exception e)
            {
                embedBuilder = new()
                {
                    ...
                    Description = GetFormattedString(InsanityBot.LanguageConfig["insanitybot.permission.error.could_not_neutralize"], ctx, member)
                };
