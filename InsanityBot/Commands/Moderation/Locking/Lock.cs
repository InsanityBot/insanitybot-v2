...
namespace InsanityBot.Commands.Moderation.Locking
{
	public class Lock : BaseCommandModule
	{
		...
		{
			await this.LockCommand(ctx, ctx.Channel, InsanityBot.LanguageConfig["insanitybot.moderation.no_reason_given"], false);
		}

		[Command("lock")]
		public async Task LockCommand(CommandContext ctx, DiscordChannel channel)
		{
			await this.LockCommand(ctx, channel, InsanityBot.LanguageConfig["insanitybot.moderation.no_reason_given"], false);
		}
		...
		{
			await this.LockCommand(ctx, args + $" -c {channel.Id}");
		}
