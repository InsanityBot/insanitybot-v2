...
namespace InsanityBot.Commands.Moderation.Locking
{
	public class Unlock : BaseCommandModule
	{
		...
		{
			await this.UnlockCommand(ctx, ctx.Channel, InsanityBot.LanguageConfig["insanitybot.moderation.no_reason_given"], false);
		}

		[Command("unlock")]
		public async Task UnlockCommand(CommandContext ctx, DiscordChannel channel)
		{
			await this.UnlockCommand(ctx, channel, InsanityBot.LanguageConfig["insanitybot.moderation.no_reason_given"], false);
		}
		...
		{
			await this.UnlockCommand(ctx, args + $" -c {channel.Id}");
		}
